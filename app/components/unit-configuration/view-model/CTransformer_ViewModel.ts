...
export class CTransformer_ViewModel implements IRenderable {
    constructor(
        private _unit: CUnit_ViewModel,
        private _model: ModelInterfacesAHU.Configuration.Types.ITransformer) {
    ...
    public get model(): ModelInterfacesAHU.Configuration.Types.ITransformer {
        return this._model;
    }
        ...
        if (this.model.unitSide === EnumsCommon.Common.E_UnitSide.Front) {
            ...
            this._exteriorMaterial_Rest_Front = PolygonGenerator.CUtility.getLabelMaterial(
                this.model.userDefinedName ? this.model.userDefinedName :
                    (this._descriptorStoreAHU.Electrical.TransformerSize.uiDescription(this.model.transformerSize) ?
                        this._descriptorStoreAHU.Electrical.TransformerSize.uiDescription(this.model.transformerSize) :
                        "XFRM"),
                "#fa9225",
                "white",
    ...
    click(evt: Event) {
        ...
        if (this.isSelected) {
            CTransformer_Selected.addToList(this.model, this);
            CTransformer_Selected.setselectedItem(this.model.id, this.isSelected);
