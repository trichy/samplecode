...
export class CListProvider_E_AirPressureType implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_AirPressureType._LIST === null) {
            CListProvider_E_AirPressureType._LIST = [];
            ...
            this._storeAHU.Common.AirPressureType.list().forEach(    
                i => CListProvider_E_AirPressureType._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));    
        }

        this.List = CListProvider_E_AirPressureType._LIST;
    }
