...
export class CListProvider_E_DoorFastenerType implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_DoorFastenerType._LIST === null) {
            CListProvider_E_DoorFastenerType._LIST = [];
            ...
            this._storeAHU.Door.DoorFastenerType.list().forEach(
                i => CListProvider_E_DoorFastenerType._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));    
        }

        this.List = CListProvider_E_DoorFastenerType._LIST;
    }
