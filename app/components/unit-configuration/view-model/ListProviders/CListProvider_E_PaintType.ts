...
export class CListProvider_E_PaintType implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_PaintType._LIST === null) {
            CListProvider_E_PaintType._LIST = [];
            ...
            this._storeAHU.ConstructionOptions.PaintType.list().forEach(
                i => CListProvider_E_PaintType._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));    
        }

        this.List = CListProvider_E_PaintType._LIST;
    }
