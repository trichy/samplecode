...
export class CListProvider_E_ControlAuxilliaryContactType implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_ControlAuxilliaryContactType._LIST === null) {
            CListProvider_E_ControlAuxilliaryContactType._LIST = [];

            if (this._storeAHU === null) {
                this._storeAHU = CGlobals.INJECTOR.get(DescriptorStoreAHU.TOKEN_IDescriptorStoreAHU);
            }

            this._storeAHU.MotorControl.ControlAuxilliaryContactType.list().forEach(
                i => CListProvider_E_ControlAuxilliaryContactType._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));
        }

        this.List = CListProvider_E_ControlAuxilliaryContactType._LIST;
    }
