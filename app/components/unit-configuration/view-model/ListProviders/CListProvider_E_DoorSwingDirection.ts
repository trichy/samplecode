...
export class CListProvider_E_DoorSwingDirection implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_DoorSwingDirection._LIST === null) {
            CListProvider_E_DoorSwingDirection._LIST = [];
            ...
            this._storeAHU.Door.DoorSwingDirection.list().forEach(
                i => CListProvider_E_DoorSwingDirection._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));    
        }

        this.List = CListProvider_E_DoorSwingDirection._LIST;
    }
