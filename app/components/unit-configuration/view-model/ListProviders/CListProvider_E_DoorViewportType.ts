...
export class CListProvider_E_DoorViewportType implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_DoorViewportType._LIST === null) {
            CListProvider_E_DoorViewportType._LIST = [];
            ...
            this._storeAHU.Door.DoorViewportType.list().forEach(
                i => CListProvider_E_DoorViewportType._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));    
        }

        this.List = CListProvider_E_DoorViewportType._LIST;
    }
