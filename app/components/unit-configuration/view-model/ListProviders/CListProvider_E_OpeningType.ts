...
export class CListProvider_E_OpeningType implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_OpeningType._LIST === null) {
            CListProvider_E_OpeningType._LIST = [];
            ...
            this._storeAHU.Opening.OpeningType.list().forEach(
                i => CListProvider_E_OpeningType._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));    
        }

        this.List = CListProvider_E_OpeningType._LIST;
    }
