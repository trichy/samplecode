...
export class CListProvider_E_MaterialStyle implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_MaterialStyle._LIST === null) {
            CListProvider_E_MaterialStyle._LIST = [];
            ...
            this._storeAHU.ConstructionOptions.MaterialStyle.list().forEach(
                i => CListProvider_E_MaterialStyle._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));    
        }

        this.List = CListProvider_E_MaterialStyle._LIST;
    }
