...
export class CListProvider_E_OpeningShape implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_OpeningShape._LIST === null) {
            CListProvider_E_OpeningShape._LIST = [];
            ...
            this._storeAHU.Opening.OpeningShape.list().forEach(
                i => CListProvider_E_OpeningShape._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));    
        }

        this.List = CListProvider_E_OpeningShape._LIST;
    }
