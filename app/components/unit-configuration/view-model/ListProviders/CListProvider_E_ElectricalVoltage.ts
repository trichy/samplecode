...
export class CListProvider_E_ElectricalVoltage implements PG.Types.IListProvider {
    ...
    public Init(): void {
        if (CListProvider_E_ElectricalVoltage._LIST === null) {
            CListProvider_E_ElectricalVoltage._LIST = [];

            if (this._storeAHU === null) {
                this._storeAHU = CGlobals.INJECTOR.get(DescriptorStoreAHU.TOKEN_IDescriptorStoreAHU);
            }

            this._storeAHU.Electrical.ElectricalVoltage.list().forEach(
                i => CListProvider_E_ElectricalVoltage._LIST.push(
                    new PG.Types.CListItem(i.uiDescription, i.enum)));
        }

        this.List = CListProvider_E_ElectricalVoltage._LIST;
    }
